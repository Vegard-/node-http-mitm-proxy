...
Proxy.prototype.onConnection = function(fn) {
  this.onConnectionHandlers.push(fn);
  return this;
};
...
Proxy.prototype._onHttpServerConnect = function(req, socket, head) {
  ...
  return self._onConnection(remoteInfo, function(err, results) {
    if (err) {
      return self._onError("ON_CONNECTION_ERROR", null, err);
    }
    var makeDirectConnection = results.reduce(function(r1, r2){ return r1 || r2; }, false);
    if (makeDirectConnection) {
      self._connectDirectly(req, socket, head);
    } else {
      self._connectAndInterceptTraffic(req, socket, head);
    }
  });
}

Proxy.prototype._connectDirectly = function(req, socket, head) {
  var self = this;
  var dest = req.url.split(':', 2);
  var hostname = dest[0];
  var port = dest[1] || 80;
  var httpVersion = req['httpVersion'];
  socket.pause();

  var destinationConnection = new net.Socket().connect(port, hostname, function() {
    socket.pipe(destinationConnection);
    destinationConnection.pipe(socket);

    destinationConnection.write(head);
    socket.write( "HTTP/" + httpVersion + " 200 Connection established\r\n\r\n" );
    return socket.resume();
  });
  destinationConnection.on('error', self._onError.bind(self, "PROXY_TO_HOST_SOCKET_ERROR", null));
};

Proxy.prototype._connectAndInterceptTraffic = function(req, socket, head) {
  var self = this;
    ...
    return self._onResponse(ctx, function(err) {
      ...
      return self._onResponseHeaders(ctx, function (err) {
        ...
        ctx.responseFilters.forEach(function(filter) {
          ...
          prevResponsePipeElem.on('error', function(err){
            return self._onError('ON_RESPONSE_ERROR', ctx, err);
          });
        });
...
Proxy.prototype._onConnection = function(remoteInfo, callback) {
  async.map(this.onConnectionHandlers, function(fn, callback) {
    return fn(remoteInfo, callback);
  }, callback);
};
